...
export function calcHybridColumns(grid, summedAppliances) {
  ...
  const columnReducer = (result, homerRow, rowIndex, homerRows) => {
    ...
    const originalModeledUnmetLoad = _.round(homerRow['originalModeledUnmetLoad'], 6)
    ...
    if (newAppliancesUnmetLoad < 0) {
      console.log('negative newAppliancesUnmetLoad: ', newAppliancesUnmetLoad)
    }
    ...
    const originalModeledExcessProduction = _.round(homerRow['originalModeledExcessProduction'])

    // This should be roughly true, given a perfect battery model: original + newAppliances = total
    // Therefore: newAppliances = total - original
    // 'original' should always be greater than or equal to the new total, since new appliances
    // will reduce the excess capacity.
    // newAppliancesExcessProduction, totalExcessProduction and originalModeledExcessProduction
    // are always positive.
    // Since totalExcessProduction is based on the battery prediction model, it makes
    // more sense to use the originalModeledExcessProduction to calculate the
    // excess due to new appliances
    ...
    if (newAppliancesExcessProduction < 0) {
      console.log('negative newAppliancesExcessProduction: ', newAppliancesExcessProduction)
    }
    ...
    result.push({
      ...
      // totalExcessNegativeProduction: _.round(totalExcessNegativeProduction, 4), //TODO: Just for debugging
    })
    return result
  }
